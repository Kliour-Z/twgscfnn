...
function method8zaosheng(finame,p)
% 代码参数设置
close all
L = 0.8; % 训练集占比
nn = 3; % 并行池数量
...
lenthofpath = 10;
rng(1213); 
method = "TWGSCFNNzaosheng(301020)"+p;
targetFolder = "D:\result\output_" + method;
mkdir(targetFolder);
%% 数据准备
...
% 将指标写入txt
fileID = fopen(fullfile(targetFolder,"resultmatlab_"+method+".txt"), 'a');
fprintf(fileID, '%s\n',finame + ":");
